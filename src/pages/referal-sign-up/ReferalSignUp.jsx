...
const ReferalSignUp = ({baseUrl}) => {
    ...
    async function handleUserSignUp(e){
        ...
        if(!email || !password || !phone || !name || !confirmPassword){
            ...
            console.log(JSON.stringify({email, password, phone, username:name, referredBy:userid}))
            setIsLoading(true)
            const response = await fetch(`${baseUrl}/user/create-account`,{
                method:"POST",
                body:JSON.stringify({email, password, phoneNumber:phone, username:name, referredBy:userid}),
                headers:{
  ...
  return (
    ...
    <div className='flex items-center justify-center flex-col bg-[#fff] my-[2rem] p-5 mx-auto w-[100%] sm:w-[80%] md:w-[60%] lg:w-[40%]'>
        <nav className='flex items-center justify-between w-full pb-1 relative flex-col shadow-md'>
            <img src={Logo} alt="" width="30%" className='mb-2'/>
            {/* <div className='bg-[#4f3d3d] text-white w-full text-center rounded-[10px] mb-5 p-[1rem]'> */}
                <h4 className='mb-[1rem] mt-[1rem] font-bold text-xl text-[#847777]'>Create Account</h4>
            {/* </div> */}
            <form className="flex items-center justify-center flex-col bg-[#fff] p-5 mx-auto w-[100%] sign-up-form relative" onSubmit={handleUserSignUp}>
                ...
                <div>
                    ...
                    <input type="text" className='cursor-not-allowed text-slate-400' value={userid}/>
                </div>
